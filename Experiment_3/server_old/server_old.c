...
int main(int argc, char **argv)
{
    ...
    // 创建线程等待用户对服务器的指令
    char choice = 'y';
    pthread_t wait_for_choice;
    pthread_create(&wait_for_choice, NULL, read_choice, (void *)&choice);
    pthread_detach(wait_for_choice);
...
void *handle_connection(void *arg)
{
    ...
    while (task != NULL)
    {
        ...
        // 创建新线程
        pthread_t tid;
        pthread_create(&tid, NULL, handle_task, (void *)argv);
        task = next;
